...
export class BranchEditComponent implements OnInit {
  ...
  fg_newbranch = this.fb.group({
    fc_name    : 'Chi đoàn tự phát Công nghệ thông tin A',
    fc_leader  : '001',
  ...
  display_members : any = [];
  display_picture : any ='/assets/imgs/logodoan.png';

  constructor(private fb : FormBuilder) { }

  ngOnInit(): void {
    console.log("listMembers value: " , this.listMembers.value);
    for (let m of this.listMembers.controls){
      console.log("Control : ", m.value)
    }
    this.getArrayToDisplay();
  }

  loadPicture(_url : string){
    this.display_picture = '/assets/imgs/' + _url;
  }

  getArrayToDisplay(){
    let mockup_array = [
      { 'm_id' : '001', 'm_avatar' : 'doanvien_nam4.jpg', 'm_studentcode' : '42.01.104.122', 'm_fullname' : 'Huỳnh Trương Chí Quân', 'm_classname' : 'Công nghệ Thông tin A', 'm_facultyname' : 'Công nghệ Thông tin'  },
      { 'm_id' : '002', 'm_avatar' : 'doanvien_nu4.jpg',  'm_studentcode' : '42.01.104.123', 'm_fullname' : 'Bùi Phương Dung',       'm_classname' : 'Văn hóa phương Đông A', 'm_facultyname' : 'Nhân văn - Xã hội'    },
      { 'm_id' : '003', 'm_avatar' : 'doanvien_nam3.jpg', 'm_studentcode' : '42.01.104.124', 'm_fullname' : 'Trần Quý Bửu',          'm_classname' : 'Văn hóa Nhật Bản A',    'm_facultyname' : 'Nhân văn - Xã hội'    },
      { 'm_id' : '004', 'm_avatar' : 'doanvien_nam2.jpg', 'm_studentcode' : '42.01.104.125', 'm_fullname' : 'Trần Sơn Đạt',          'm_classname' : 'Âm nhạc B',             'm_facultyname' : 'Âm nhạc'              },
      { 'm_id' : '005', 'm_avatar' : 'doanvien_nam1.jpg', 'm_studentcode' : '42.01.104.126', 'm_fullname' : 'Lê Quý Kỳ Phong',       'm_classname' : 'Kiến trúc A',           'm_facultyname' : 'Kiến trúc - Xây dựng' },
      { 'm_id' : '006', 'm_avatar' : 'doanvien_nam5.jpg', 'm_studentcode' : '42.01.104.127', 'm_fullname' : 'Nguyễn Thanh Tân',      'm_classname' : 'Kiến trúc B',           'm_facultyname' : 'Kiến trúc - Xây dựng' }
    
    ];
    for (let m of mockup_array){
      this.listMembers.push(this.fb.group({
        m_id          : m.m_id,
        m_avatar      : m.m_avatar,
        m_studentcode : m.m_studentcode,
        m_fullname    : m.m_fullname,
        m_classname   : m.m_classname,
        m_facultyname : m.m_facultyname
      }))
    }
